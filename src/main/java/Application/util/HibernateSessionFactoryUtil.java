    ...
    public static Session getSessionFactory() { // метод содержащий в себе параметры для подключения к БД
        try {
            if (sessionFactory == null) { //если сессия пустая/еще не существует
                ...
                Configuration configuration = new Configuration().addAnnotatedClass(UsersChat.class)
                        .addAnnotatedClass(Workout.class).addAnnotatedClass(GroupTable.class).addAnnotatedClass(StatisticWorkout.class).addAnnotatedClass(Exercises.class)
                        .setProperty(Environment.DRIVER, "org.postgresql.Driver")
                        .setProperty(Environment.URL, "jdbc:postgresql://localhost:5432/TelegramMOS_221BOT")
